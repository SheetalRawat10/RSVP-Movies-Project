...
JOIN genre AS g
  ON m.id = g.movie_id
WHERE
  r.total_votes > 1000 AND m.date_published BETWEEN '2017-03-01' AND '2017-03-31'
GROUP BY
  g.genre;
...
-- Type your code below:
SELECT
    AVG(ratings.total_votes) AS average_votes,
    CASE
        WHEN movie.country = 'Germany' THEN 'German'
        WHEN movie.country = 'Italy' THEN 'Italian'
        ELSE 'Other'
...
WHERE
    movie.country IN ('Germany', 'Italy')
GROUP BY
    movie_country;
...
WITH Top_Genres AS (
    SELECT
        g.genre,
        COUNT(DISTINCT m.id) AS movie_count_genre
    FROM
        movie AS m
    JOIN
    ...
    GROUP BY
        g.genre
    ORDER BY
        movie_count_genre DESC
    LIMIT 3
),
Top_Directors_In_Top_Genres AS (
  SELECT
    ...
    WHERE
        r.avg_rating > 8 AND g.genre IN (SELECT genre FROM Top_Genres)
    GROUP BY
        director_name
    ORDER BY
        movie_count DESC
...
SELECT * FROM Top_Directors_In_Top_Genres;
...
WITH Actor_Ratings AS (
    ...
    JOIN
        ratings r ON m.id = r.movie_id
    WHERE
        m.country = 'India' AND rm.category = 'actor'
    GROUP BY
        n.name
    HAVING
...
WITH Actress_Ratings AS (
    ...
    JOIN
        ratings r ON m.id = r.movie_id
    WHERE
        m.country = 'India' AND m.languages = 'Hindi' AND rm.category = 'actress'
    GROUP BY
        n.name
    HAVING
...
SELECT
    ...
    SUM(m.duration) OVER (PARTITION BY g.genre ORDER BY m.duration) AS running_total_duration,
    AVG(m.duration) OVER (PARTITION BY g.genre ORDER BY m.duration) AS moving_avg_duration
FROM
    movie m
JOIN
    genre g ON m.id = g.movie_id
...
top_movies AS (
    SELECT
        ...
        r.total_votes * r.avg_rating AS worldwide_gross_income,
        ROW_NUMBER() OVER (PARTITION BY m.year, g.genre ORDER BY r.total_votes * r.avg_rating DESC) AS movie_rank
    FROM
        movie m
    JOIN
        genre g ON m.id = g.movie_id
...
-- Type your code below:
 WITH top_production_houses AS (
    ...
    JOIN
        ratings r ON m.id = r.movie_id
    WHERE
        m.languages != 'Single' AND r.median_rating >= 8
    GROUP BY
        m.production_company
)
SELECT
...
InterMovieDurations AS (
    SELECT
        director_id,
        AVG(diff_between_date_published) AS avg_inter_movie_days
    FROM (
        SELECT
            dm.name_id AS director_id,
            TIMESTAMPDIFF(DAY, LAG(m.date_published) OVER (PARTITION BY dm.name_id ORDER BY m.date_published), m.date_published) AS diff_between_date_published
        FROM
            director_mapping dm
        JOIN
            movie m ON dm.movie_id = m.id
    ) AS diff_between_date_published
    GROUP BY
        director_id
)
SELECT
...
FROM
    DirectorMovies dm
JOIN
    InterMovieDurations imd ON dm.director_id = imd.director_id
ORDER BY
